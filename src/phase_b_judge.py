...
if __name__ == "__main__":
    ...
    with open(ANSWERS_PATH, encoding="utf-8") as f:
        answers_data = {item["id"]: item["ground_truth"] for item in json.load(f)}

    human_labels = []
    judge_labels = []
    judge_results = []
    
    print(f"Loaded {len(human_data)} questions for evaluation.")
    
    for item in human_data:
        qid = item["question_id"]
        q = item["question"]
        a_a = item["model_answer"]
        a_b = answers_data.get(qid, "Không có ground truth.")
        
        result = swap_and_average(q, a_a, a_b)
        judge_results.append(result)
        
        # If model_answer (A) wins or ties with ground_truth (B), it's a good answer (1)
        j_label = 1 if result.final_winner in ("A", "tie") else 0
        ...
        "evaluations": [
            {
                ...
                "human_label": hd["human_label"],
                "judge_label": jl,
                "final_winner": jr.final_winner,
                "position_consistent": jr.position_consistent
            }
            for hd, jl, jr in zip(human_data, judge_labels, judge_results)
        ]
    }
