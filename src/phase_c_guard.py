...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    response_data = await rails.generate_async(
        messages=[{"role": "user", "content": text}]
    )
    # Extract string content from the response dict
    ...
    # Heuristic fallback due to mock LLM failure (returning empty)
    bad_keywords = ["admin", "dan", "ignore", "override", "bitcoin", "phim", "thơ", "phương trình", "tấn công", "bảng lương"]
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    response_data = await rails.generate_async(messages=[
